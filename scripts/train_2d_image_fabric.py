...
def load_fabric_dataset(root_path):
    img_ls=[]
    import cv2
    for f_img in os.listdir(root_path):
        img=cv2.imread(os.path.join(root_path,f_img))
        img_rgb=cv2.cvtColor(img,cv2.COLOR_BGR2RGB)/255.0
        img_ls.append(img_rgb)
    dataset = {
        "data_test": np.stack(img_ls,axis=0)
    }
    return dataset
...
def main(args):
    ...
    elif args.dataset == "fabrics":
        dataset = load_fabric_dataset('./data/fabrics')
        data_channels = 3
        RES = 280
    ...
    elif args.dataset in ["fabrics"]:
        full_x_grid*=2
        mask_train=torch.zeros_like(full_x_grid[0,:,:,0])
        mask_train[RES//2-RES//4:RES//2+RES//4,RES//2-RES//4:RES//2+RES//4]=1
        mask_train=mask_train.bool()
        x_train = full_x_grid[:, RES//2-RES//4:RES//2+RES//4,RES//2-RES//4:RES//2+RES//4]
        x_test = full_x_grid

        y_train = dataset["data_test"][:, RES//2-RES//4:RES//2+RES//4,RES//2-RES//4:RES//2+RES//4]
        y_test = dataset["data_test"][:,]
    else:
    ...
    if args.cfg_file is not None:
        with open(os.path.join("./fabrics_configs/", args.cfg_file)) as f:
            cfg = CN.load_cfg(f)
        cfg_list.append(cfg)
    else:
        for conf_file in os.listdir("./fabrics_configs/"):
            with open(os.path.join("./fabrics_configs/", conf_file)) as f:
                cfg = CN.load_cfg(f)
            cfg_list.append(cfg)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dataset", default="nature", choices=["camera", "celeba", "nature", "text","fabrics"]
    )
